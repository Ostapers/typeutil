      ...
      struct
	    ...
	    method render (back, backA, backCb) (next, nextA, nextCb) = 
	      ...
	      in 
		     ...
		     | Item.Div _ -> generate (sprintf "  if (curr[\"%s\"]) document.getElementById (\"%s\").innerText = curr[\"%s\"];\n" name id name);
                     | Item.Radio _ ->
              ...
              generate (sprintf "function %s (curr) {\n" savef);
                ...
                List.iter 
                  (fun (name, id, t) -> 
                     ...
                     | Item.Div   _ -> generate (sprintf "  curr[\"%s\"] = document.getElementById(\"%s\").innerText.replace(/\\u00a0/g, \" \");\n" name id)
                     | Item.Radio _ -> 
                       generate (sprintf "  coll = document.getElementsByName (\"%s\");\n" id);
                       generate          "  for (var i = 0 ; i<coll.length; i++) {\n";
